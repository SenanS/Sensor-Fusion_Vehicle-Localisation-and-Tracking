function [TPHAT] = SFlabFindPulseTimes(recBuffer)
  %
  ...
  %Parameters - update if any changes!!
  recChanList  =  [1,2,3,4,5,6,7,8]; %vector of recording channels
  playChanList = 1;  %Vector with the output channels that were used.
  sampRate     =44100;  %Sampling frequency.
  pulseFreq    =[790, 800, 810, 820, -830, 840, 850, -860, 870, -880, -890, 900, 910, 920, -930, 940, -950, -960, -970, -980, 990, -1000, 1010, -1020, 1030, -1040, 1050, 1060, 1070, -1080, -1090, -1100, 1110, -1120, 1130, 1140, 1150, -1160, -1170, -1180]; %Vector with pulse frequencies.
  pulseWidth   =0.1; %width of the pulses in seconds.
  npt          =2; %number of pulses per second for each output channel.
  time         =length(recBuffer)/sampRate;  %Length in seconds of the recording.
  plotOn       =1;  % Plot option, set 1 to plot, 0 otherwise.
  soundSeq     =[];  %Matrix containing sound that was played. [] if external
  %                 speaker was used.
  ignorePolarity =true;  %If polarity should be ignored when detecting pulses or
  %                 not.  Default: true
  %
  % OUTPUTS:
